...
class UpdateRentingPage extends StatefulWidget {
  ...
  const UpdateRentingPage(
      ...
      required this.myRentingModel});

  @override
  // ignore: library_private_types_in_public_api
  State<UpdateRentingPage> createState() => _UpdateRentingPageState();
}

class _UpdateRentingPageState extends State<UpdateRentingPage> {
  ...
  void initState() {
    ...
    if (widget.myRentingModel.checkIn != null &&
        widget.myRentingModel.checkIn != null) {
      _rangeDatePickerValueWithDefaultValue = [
        DateTime.parse(widget.myRentingModel.checkIn!),
        DateTime.parse(widget.myRentingModel.checkOut!),
      ];
    }

    if (widget.myRentingModel.children != null) {
      _childrenController.text = widget.myRentingModel.children.toString();
    }

    if (widget.myRentingModel.adult != null) {
      _adultsController.text = widget.myRentingModel.adult.toString();
    }

    if (widget.myRentingModel.totalFamilyMember != null) {
      _totalFamilyMembersController.text =
          widget.myRentingModel.totalFamilyMember.toString();
    }
    super.initState();
  }
  ...
  Widget _buildScrollRangeDatePickerWithValue() {
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          Text(
              'Total Price: ${widget.property.currency} ${intl.NumberFormat('#,##0.00', widget.property.currency == "Tsh" ? "sw_TZ" : "en_US").format(double.parse(widget.totalPrice.toString()))}',
              ...
              children: [
                ...
                BlocListener<MyRentingBloc, MyRentingState>(
                  listener: (context, state) {
                    if (state is UpdateMyRentingSuccess) {
                      showMessage(
                    ...
                    if (state is UpdateMyRentingFailed) {
                      showMessage(
                          message: state.message,
                          title: "Error",
                          isAnError: true);
                    ...
                    if (state is UpdateMyRentingLoading) {
                      isAdding = true;
                    }
                  },
                    ...
                    onPressed: isAdding
                        ...
                        : () {
                            ...
                            if (cIn == 'null' || cOut == 'null') {
                              showMessage(
                                  message: "Please select check-in - check-out",
                                  title: "Error",
                                  isAnError: true);
                              return;
                            }
                            ...
                            BlocProvider.of<MyRentingBloc>(context)
                                .add(UpdateMyRenting(
                              rentingId: widget.myRentingModel.id,
